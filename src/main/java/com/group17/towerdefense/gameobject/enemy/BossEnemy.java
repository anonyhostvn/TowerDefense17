...
public class BossEnemy extends AbstractGroundEnemy {

    public BossEnemy (Point position, GameField gameField) {
        this.absVelocity = Config.BOSS_ENEMY_ABS_VELOCITY;
        this.width = Config.BOSS_ENEMY_WIDTH;
        this.height = Config.BOSS_ENEMY_HEIGHT;
        this.position = new Point(position);
        this.vectorVelocity = new DescartesVector(0,0);
        this.health = Config.BOSS_ENEMY_INIT_HEALTH;
    ...
    public void beAttacked(int damage) {
        this.health -= damage;
        if (this.health <= 0) this.recentGameField.getRecentStage().changeCoins(Config.BOSS_ENEMY_AWARD);
    }
